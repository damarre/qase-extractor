...
const {TestCaseFilters} = require("../../node_modules/qaseio/dist/src/models/cases.js")
...
// Get all test suites
qase.suites.getAll(projectName, {limit: 3, offset: 0}).then((res) => {
  ...
  testSuites.forEach(function (item, index) {
    if (item.parent_id == null){
      ...
      shell.mkdir('-p', './__tests__/features/' + item.title);
        ...
        qase.cases.getAll(projectName, { limit: 3, offset: 0, filters: new TestCaseFilters({suite_id : item.id}) }).then((cases) => {
        // crate file feature based on item id
        console.log(cases.data.entities.length, item.id)
        if (cases.data.entities.length != 0){
          cases.data.entities.forEach(function (data, index) {
            ...
            fs.appendFileSync('./__tests__/features/' + item.title + '/' +  projectName + '-' + data.id + '.feature' , '@' + item.title+ ' ', 'utf8');
      ...
      qase.suites.get(projectName, item.parent_id).then((res) => {
        ...
        qase.cases.getAll(projectName, { limit: 100, offset: 0, filters: new SuiteFilters({suite_id : item.id}) }).then((cases) => {
          console.log('====================================')
          // crate file feature
          if (cases.data.entities.length != 0){
            cases.data.entities.forEach(function (data, index) {
