"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Milestones = void 0;
var models_1 = require("../models");
var _1 = require(".");
var Milestones = /** @class */ (function (_super) {
    __extends(Milestones, _super);
    function Milestones() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    Milestones.prototype.getAll = function (code, _a) {
        var limit = _a.limit, offset = _a.offset, filters = _a.filters;
        var filterParams = new models_1.Filter(filters === null || filters === void 0 ? void 0 : filters.filter).filter();
        return this.api
            .get("/milestone/" + code, { params: __assign({ limit: limit, offset: offset }, filterParams) })
            .then(this.validateResponse());
    };
    Milestones.prototype.get = function (code, milestoneId) {
        return this.api
            .get("/milestone/" + code + "/" + milestoneId)
            .then(this.validateResponse());
    };
    Milestones.prototype.exists = function (code, milestoneId) {
        return this.get(code, milestoneId)
            .then(function (resp) { return Boolean(resp.data.id); })
            .catch(function () { return false; });
    };
    Milestones.prototype.create = function (code, data) {
        return this.api
            .post("/milestone/" + code, data)
            .then(this.validateResponse());
    };
    Milestones.prototype.update = function (code, milestoneId, data) {
        return this.api
            .patch("/milestone/" + code + "/" + milestoneId, data)
            .then(this.validateResponse());
    };
    Milestones.prototype.delete = function (code, milestoneId) {
        return this.api
            .delete("/milestone/" + code + "/" + milestoneId)
            .then(this.validateResponse());
    };
    return Milestones;
}(_1.BaseService));
exports.Milestones = Milestones;
//# sourceMappingURL=milestones.js.map